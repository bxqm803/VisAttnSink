...
def build_prompt_for_caption(model, cfgs, caption):
    """
    Build a LLaVA prompt that contains the image token and one candidate caption.

    We score the candidate caption by masking the prefix and computing LM loss
    only on the caption tokens.
    """
    ...
    # Prefix prompt with no caption, used only to estimate mask length.
...
def count_target_tokens(input_ids, prefix_len):
...
def score_caption_option(
    ...
    """
    Return mean_logprob and sum_logprob for one caption option.

    score_mode:
        mean: compare average log likelihood per target token.
        sum:  compare total log likelihood.
    """
    ...
    # HF causal LM loss is mean cross entropy over non-masked labels.
    mean_logprob = -loss
        ...
        for idx, line in enumerate(tqdm(questions)):
            try:
                ...
                for option_idx, caption in enumerate(caption_options):
                    ...
                    score = rec["mean_logprob"] if score_mode == "mean" else rec["sum_logprob"]
